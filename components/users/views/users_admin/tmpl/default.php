...
$arr = array(1=>'Approved', 0=>'Approve');
...
<div class="com-head">
	<h3>Users: Admin approval awaiting</h3>
</div>
<div class="form"><?php
	//$list = array();
	//$list['view']='country';
...
?></div>
	...
	<table id="data-table" class="table">
		<thead><tr><th>ID#</th><th>Owner</th><th>Subscription</th><th>Approval</th><th>Actions</th></tr></thead>
...
$(".approve").click(function(e){
	...
	$.post("index.php?com=vehicles&view=users",
		{action: "approveUser",	id: user},
	function(data, status){
		...
		$(msg).css('opacity',1);
		if(data.error){
			$(msg).html('<div class="alert alert-danger">' + data.error.msg + '</div>').stop().fadeOut(2000);
		}else{
			//$(r).attr("disabled", true);
			$(msg).html('<div class="alert alert-success">User approved</div>').stop().fadeOut(2000, function(){
				$(r).remove();
			});
		}
	});
