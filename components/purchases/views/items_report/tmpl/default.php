...
<div class="table-responsive"><div id="search-date-range">
		<form method="post" action="?com=purchases&view=items_report" >
			...
			<div class="date-range">
				...
				<input type="date" name="start_date" id="start_date"class="inputbox input-sm date<?php if(!isset($_POST['start_date'])){ echo '-default';}?>" value="<?php if(isset($_POST['start_date'])){ echo $_POST['start_date'];}?>" tabindex="-1" />
				<label class="control-label" for="end_date">End date:</label>
				<input type="date" name="end_date" id="end_date" class="inputbox input-sm date<?php if(!isset($_POST['end_date'])){ echo '-default';}?>" value="<?php if(isset($_POST['end_date'])){ echo $_POST['end_date'];}?>" tabindex="-1" />
	...
	<table id="purchasesReportItems" class="table table-bordered table-hover table-condenseds">
		<thead>
		<tr>
			<th>Item#</th><th>Title</th><th>Unit</th><th>Pur. Price</th>
		...
		<tfoot>
			<tr>
				...
				<td id="subTotal"><b></b></td><td id="subTotalMargin"><b></b></td>
			</tr>
		</tfoot>
	...
	?>;
		...
		$.each(json_data, function(i, v) {
			...
			//document.write('<td class="cat">'+ v.cat_title+'</td>');
