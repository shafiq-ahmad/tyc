...
?></div>
	...
	<table id="data-table" class="table">
		<thead><tr><th>Image</th><th>Title</th><th>Owner</th><th>Approval</th><th>Actions</th></tr></thead>
		...
		?><tr <?php if($this->id==$row['id']){echo ' class="active"';} ?>><?php 
			//echo '<td>' . $row['id'] . '</td>';
			echo '<td><img src="' . $row['img_thumb'] . '" class="img-thumbnail" alt="No image" style="max-width:100px;" /></td>';
			echo '<td>' . $row['title'] . '</td>'; 
			echo '<td>' . $row['owner_name'] . '</td>'; 
			//echo '<td>' . $row['sub_name'] . '</td>';
			if($row['visibility']==3){ //approval old feature is disabled by changing class approval to approval1
				echo '<td><a class="approve1 btn btn-warning btn-sm disabled" data-id="' . $row['id'] . '" disabled>' . $arr[$row['visibility']] . '</a></td>'; 
			}else{
				echo '<td>' . $arr[$row['visibility']] . '</td>';
			}
			$edit_link = "?com=vehicles&view=vehicle_admin&id={$row['id']}&task=edit";
			echo '<td>'; 
				echo '<a href="' . $edit_link . '" title="Edit" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>'; 
			//if(!$row['cnt']){
