...
?></div>
	<div>
		<span><a href="index.php?com=vehicles&view=make&task=edit" class="btn btn-info btn-flat btn-sm" tabindex="-1" ><i class="fa fa-plus"></i> New</a></span>
	</div>
	...
	<table id="data-table" class="table">
		...
		?><tr <?php if($this->id==$row['id']){echo ' class="active"';} ?>><?php 
			...
			echo '<td>'; 
				echo '<a class="btn btn-info btn-flat btn-sm" href="' . $edit_link . '" title="Edit"><i class="fa fa-edit"></i></a>'; 
			//if(!$row['cnt']){
				$del_link = "index.php?com=vehicles&view=make&id={$row['id']}&task=edit";
				echo '&nbsp;&nbsp; | &nbsp;&nbsp;<a class="btn btn-danger btn-flat btn-sm" href="#" onclick="return removeItem(' . $row['id'] . ",'" . $del_link . '\')" title="Delete"><i class="fa fa-remove"></i></a>'; 
			//}
			echo '</td>'; 
		?></tr><?php
			}
